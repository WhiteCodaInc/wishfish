...
class Paypal extends CI_Controller {
    ...
    function index() {
        ...
        $this->paypal->set_acct_info($gatewayInfo->api_username, $gatewayInfo->api_password, $gatewayInfo->api_signature);
        ...
        $response = $this->paypal->request('SetExpressCheckout', $requestParams + $recurring);

        if (is_array($response) && $response['ACK'] == 'Success') { //Request successful
            $token = $response['TOKEN'];

            if ($this->paypal->is_sandbox)
    ...
    function consolidate() {
        ...
        if ($this->input->get('token')) { // Token parameter exists
            ...
            $checkoutDetails = $this->paypal->request('GetExpressCheckoutDetails', array('TOKEN' => $this->input->get('token')));
